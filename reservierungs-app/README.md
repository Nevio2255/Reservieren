...
## 2. web.de App-Passwort erstellen

1. Bei web.de einloggen → **Einstellungen → Sicherheit → App-Passwörter** (oder
   "POP3/IMAP-Zugang", je nach Kontotyp) → neues App-Passwort erstellen.
2. IMAP- und SMTP-Zugriff für das Postfach aktivieren, falls gefragt.
3. Das erzeugte App-Passwort notieren (nicht das normale Login-Passwort!).
...
- **E-Mail senden**: Compose-Feld in der Detailansicht einer Reservierung →
  verschickt über dein web.de-Postfach → wird im Verlauf gespeichert.
- **E-Mail empfangen**: alle 10 Minuten prüft eine automatische Funktion das
  web.de-Postfach, ordnet neue Mails per Absenderadresse der passenden
...
- **PDF-Rechnung**: Beim Senden der „Bestätigung"-Vorlage wird automatisch eine PDF-Rechnung erzeugt und angehängt (Reservierungsnummer, Kunde, Produkt, Preis).
- **Reihum-Zuweisung**: Jede neue Reservierung wird automatisch reihum einem Mitarbeiter zugewiesen ("Zuständig"-Spalte in der Tabelle und Detailansicht).
- **Ton & Browser-Benachrichtigung**: Bei neuer Chat-Nachricht oder neuer Kunden-E-Mail gibt's einen kurzen Ton; wenn du Benachrichtigungen erlaubst (Button unten links in der Sidebar), auch eine Desktop-Benachrichtigung — funktioniert auch, wenn der Tab im Hintergrund ist.
- **Posteingang als Kanäle**: Zeigt jetzt alle E-Mail-Unterhaltungen (zugeordnete und unzugeordnete) als Liste mit Kundenname, Klick öffnet die Unterhaltung mit Antwortfeld — genau wie bei Live-Chats. Neue Kunden-Mails kommen aber weiterhin nur alle 30 Minuten vom Postfach rein (siehe `netlify.toml`).
