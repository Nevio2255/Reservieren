...
// web.de-Postfach per IMAP, ordnet sie einer Reservierung zu (per Absender-Adresse)
  ...
  try {
    ...
    try {
      ...
      for await (const message of client.fetch(range, { source: true, uid: true }, { uid: true })) {
        ...
        // Nur Mails von echten Kunden (= Absender passt zu einer Reservierung)
        // landen im Posteingang. Newsletter, Benachrichtigungen etc. werden
        ...
        if (matchedReservation?.id) {
          await supabase.from('emails').insert({
            ...
            to_address: process.env.WEBDE_EMAIL,
            subject,
            body,
            is_read: false,
          });
