...
// Läuft automatisch alle 2 Minuten (siehe netlify.toml), holt neue Mails aus dem
// Gmail-Postfach per IMAP. Nur Mails von Adressen, die zu einer bestehenden
// Reservierung gehören, werden gespeichert — alles andere (Newsletter, Spam,
// private Mails) wird beim Abrufen ignoriert.
  ...
  try {
    ...
    try {
      ...
      for await (const message of client.fetch(range, { source: true, uid: true }, { uid: true })) {
        ...
        // Mails, die zu keiner Reservierung gehören, werden komplett ignoriert
        // (nicht gespeichert) — nur echte Kunden-Antworten landen im Posteingang.
