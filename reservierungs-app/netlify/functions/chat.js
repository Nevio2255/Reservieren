...
const { sendBrandedEmail } = require('./_mailer');
...
async function resolveAccess(event, params) {
  if (params.token) {
    const { data: chat, error } = await supabaseAdmin.from('live_chats').select('*').eq('token', params.token).maybeSingle();
    if (error) { console.error('resolveAccess-Fehler (token):', error); return { dbError: error.message }; }
    if (!chat) return null;
    return { chat, as: 'customer' };
  }
  const session = getSession(event);
  if (!session || !params.chatId) return null;
  const { data: chat, error } = await supabaseAdmin.from('live_chats').select('*').eq('id', params.chatId).maybeSingle();
  if (error) { console.error('resolveAccess-Fehler (chatId):', error); return { dbError: error.message }; }
  if (!chat) return null;
  if (session.role !== 'owner' && chat.employee_id !== session.id) return null;
  return { chat, as: 'staff', session };
}

// Liefert die passende Fehlerantwort für einen fehlgeschlagenen Zugriff, oder null wenn alles ok ist.
// Unterscheidet zwischen "wirklich nicht gefunden" (404) und einem echten Datenbankfehler (500),
// damit ein DB-Problem nicht fälschlich als "Link ungültig" angezeigt wird.
function accessErrorResponse(access) {
  if (access && access.dbError) return json(500, { error: 'Datenbankfehler: ' + access.dbError });
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    { const errRes = accessErrorResponse(access); if (errRes) return errRes; }
  ...
  if (event.httpMethod === 'POST') {
    ...
    // --- Mitarbeiter: Verifizierungscode setzen und per E-Mail an den Kunden schicken ---
    if (body.action === 'send_verification') {
      ...
      if (!reservation || !reservation.customer_email) {
        return json(400, { error: 'Für diese Reservierung ist keine E-Mail-Adresse hinterlegt.' });
      }
      ...
      const emailSubject = 'Ihr Verifizierungscode';
      const emailBody = `Guten Tag ${reservation.customer_name || ''},\n\nIhr Verifizierungscode für den Live-Chat lautet:\n\n${code}\n\nDieser Code ist 30 Minuten gültig.`;
      try {
        await sendBrandedEmail({ to: reservation.customer_email, subject: emailSubject, body: emailBody });
        await supabaseAdmin.from('emails').insert({
          reservation_id: access.chat.reservation_id,
          direction: 'out',
          from_address: process.env.GMAIL_EMAIL,
          to_address: reservation.customer_email,
          subject: emailSubject,
          body: emailBody,
          is_read: true,
        });
      } catch (err) {
        console.error('Verifizierungs-E-Mail-Fehler:', err);
      ...
      const { error: insertError } = await supabaseAdmin.from('chat_messages').insert({
        ...
        body: 'Wir haben Ihnen den Verifizierungscode gesendet. Schauen Sie bitte in Ihre E-Mails – entweder in Ihr normales Postfach oder in den Spam-Ordner.',
        request_type: 'verification', read_by_staff: true,
      });
      if (insertError) {
        console.error('Verifizierungs-Nachricht-Fehler:', insertError);
        return json(500, { error: 'Nachricht konnte nicht gespeichert werden: ' + insertError.message });
      }
    ...
    if (body.action === 'submit_verification') {
      ...
      // Erfolg der Speicherung explizit prüfen: bisher wurde ein Fehler beim
      // Freischalten (z.B. durch fehlende Berechtigungen) stillschweigend
      // ignoriert – der Kunde sah "freigeschaltet", war es aber nie wirklich,
      // und der Chat blockierte kurz danach wieder.
      const { data: verifiedRow, error: verifyUpdateError } = await supabaseAdmin
        .from('live_chats')
        .update({ verified: true })
        .eq('id', access.chat.id)
        .select()
        .maybeSingle();
      if (verifyUpdateError || !verifiedRow) {
        console.error('Freischalten fehlgeschlagen:', verifyUpdateError);
        // Den echten Datenbankfehler mit ausgeben, damit man sofort sieht, woran es liegt
        // (z.B. falscher/abgelaufener Supabase-Schlüssel, fehlende Berechtigung o.ä.).
        const detail = verifyUpdateError?.message || 'Kein Datensatz aktualisiert (evtl. Berechtigungsproblem).';
        return json(500, { error: 'Freischalten fehlgeschlagen: ' + detail });
      }
      const { error: verifyMsgError } = await supabaseAdmin.from('chat_messages').insert({
      ...
      if (verifyMsgError) console.error('Freischalt-Nachricht-Fehler:', verifyMsgError);
      return json(200, { success: true });
    }
