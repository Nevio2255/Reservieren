...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (access.as === 'staff') {
      const maintenanceBlock = await checkMaintenance(access.session);
    ...
    if (access.as === 'staff') {
      ...
      if (!access.chat.customer_joined_seen) {
        await supabaseAdmin.from('live_chats').update({ customer_joined_seen: true }).eq('id', access.chat.id);
        access.chat.customer_joined_seen = true;
      }
    }

    if (access.as === 'customer' && !access.chat.customer_joined) {
      await supabaseAdmin.from('live_chats').update({ customer_joined: true, customer_joined_seen: false }).eq('id', access.chat.id);
      access.chat.customer_joined = true;
      access.chat.customer_joined_seen = false;
    }
