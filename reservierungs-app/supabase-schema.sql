...
create table if not exists email_templates (
  ...
  attach_invoice boolean not null default false,
  created_at timestamptz not null default now()
);
...
-- Tabelle: globaler Systemzustand (aktuell nur der Wartungsmodus).
-- Wenn maintenance_mode = true, können sich Mitarbeiter (role != 'owner')
-- weder einloggen noch bereits laufende Sitzungen weiter benutzen -- nur
-- der Inhaber hat dann noch Zugriff.
...
insert into system_state (id, maintenance_mode) values (1, false) on conflict (id) do nothing;
