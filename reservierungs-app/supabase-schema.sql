...
-- Falls die Tabelle schon von einer früheren Version existiert: fehlende Spalten nachrüsten
alter table email_templates add column if not exists is_chat_invite boolean not null default false;
alter table email_templates add column if not exists attach_invoice boolean not null default false;

-- Eindeutiger Index statt "unique"-Constraint, damit das auch bei bereits bestehenden
-- Tabellen sicher nachgerüstet werden kann (wird für "on conflict (label)" unten gebraucht)
create unique index if not exists idx_email_templates_label on email_templates (label);

-- Vier Standard-Vorlagen + eine Live-Chat-Einladung vorbelegen
insert into email_templates (label, subject, body, attach_invoice) values
  ('Bestätigung', 'Deine Reservierung {{reservation_number}} ist bestätigt',
   E'Hallo {{customer_name}},\n\nvielen Dank für deine Reservierung von {{product}} (Stückzahl: {{quantity}}) bei LuxeFinds.\n\nReservierungsnummer: {{reservation_number}}\nGesamtpreis: {{total}}\n\nDie Rechnung findest du im Anhang. Wir melden uns, sobald deine Bestellung abholbereit ist.\n\nViele Grüße\nLuxeFinds', true)
...
-- Falls die Tabelle schon von einer früheren Version existiert: fehlende Spalte nachrüsten
alter table reservations add column if not exists assigned_employee_id uuid references employees(id);

-- Merkt sich, welcher Mitarbeiter zuletzt eine Reservierung zugewiesen bekam (für die
-- reihum-Zuweisung neuer Reservierungen)
create table if not exists assignment_state (
  id int primary key default 1,
  last_employee_id uuid
);
insert into assignment_state (id, last_employee_id) values (1, null) on conflict (id) do nothing;
...
create table if not exists live_chats (
  ...
  customer_joined_seen boolean not null default true,
  created_at timestamptz not null default now(),
...
-- Falls die Tabelle schon von einer früheren Version existiert: fehlende Spalten nachrüsten
alter table live_chats add column if not exists verified boolean not null default false;
alter table live_chats add column if not exists verification_code text;
alter table live_chats add column if not exists verification_expires_at timestamptz;
alter table live_chats add column if not exists customer_joined boolean not null default false;
alter table live_chats add column if not exists customer_joined_seen boolean not null default true;
...
-- Falls die Tabelle schon von einer früheren Version existiert: fehlende Spalte nachrüsten
alter table chat_messages add column if not exists request_type text;
