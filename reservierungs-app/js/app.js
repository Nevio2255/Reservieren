...
let composeAttachInvoice = false;
...
function showApp() {
  ...
  $('#maintenance-toggle-btn').classList.toggle('hidden', session.role !== 'owner');
  loadReservations();
  loadInbox();
  loadTemplates();
  loadLiveChats();
...
// ===== Wartungsmodus =====
// Solange der Inhaber "In Bearbeitung" aktiviert hat, sperrt das Backend jede
// Mitarbeiter-Anfrage (siehe checkMaintenance in den Netlify Functions). Hier
// holt sich der Client den aktuellen Stand: der Inhaber sieht/steuert den
// Schalter, ein bereits eingeloggter Mitarbeiter wird automatisch abgemeldet,
// sobald der Modus aktiviert wird.
function renderMaintenanceButton(active) {
  const btn = $('#maintenance-toggle-btn');
  btn.classList.toggle('maintenance-on', active);
  btn.classList.toggle('maintenance-off', !active);
  btn.textContent = active ? '🟠 In Bearbeitung — beenden' : '🔧 In Bearbeitung setzen';
}
...
async function loadMaintenanceState() {
  ...
  try {
    ...
    if (session.role === 'owner') {
      renderMaintenanceButton(data.maintenance);
      $('#maintenance-banner').classList.toggle('hidden', !data.maintenance);
    } else if (data.maintenance) {
      kickOutForMaintenance();
    }
  } catch (err) {
    // Falls schon ein anderer Aufruf den Mitarbeiter ausgesperrt hat (423),
    // hier ebenfalls abmelden — nicht bei sonstigen (z. B. Netzwerk-)Fehlern.
    if (session && session.role !== 'owner' && /bearbeitet/i.test(err.message || '')) {
      kickOutForMaintenance();
    }
  }
}

function kickOutForMaintenance() {
  localStorage.removeItem('session');
  session = null;
  clearInterval(globalPollTimer);
  showLogin();
  $('#login-error').textContent = 'Der Inhaber bearbeitet gerade das System. Du wurdest abgemeldet.';
}

$('#maintenance-toggle-btn').addEventListener('click', async () => {
  const turningOn = !$('#maintenance-toggle-btn').classList.contains('maintenance-on');
  if (turningOn && !confirm('System auf "In Bearbeitung" setzen? Mitarbeiter können sich währenddessen nicht einloggen und werden aus laufenden Sitzungen abgemeldet.')) {
    return;
  }
  const btn = $('#maintenance-toggle-btn');
  btn.disabled = true;
  try {
    const data = await api('system-state', { method: 'POST', body: { maintenance: turningOn } });
    renderMaintenanceButton(data.maintenance);
    $('#maintenance-banner').classList.toggle('hidden', !data.maintenance);
  } catch (err) {
...
$$('.nav-item').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    if (view === 'livechats') loadLiveChats();
...
function renderReservations(list) {
  ...
  list.forEach((r) => {
    ...
    tr.innerHTML = `
      ...
      <td>${r.quantity}</td>
      <td>${euro(r.price)}</td>
      <td class="muted">${escapeHtml(r.assigned_employee_name) || '–'}</td>
...
$('#new-reservation-btn').addEventListener('click', () => {
  $('#new-res-overlay').classList.remove('hidden');
  const numberInput = $('#new-res-number');
  numberInput.value = 'LF-';
  numberInput.focus();
...
// Sorgt dafür, dass das "LF-" Präfix nicht versehentlich gelöscht/verändert wird
$('#new-res-number').addEventListener('input', (e) => {
  if (!e.target.value.startsWith('LF-')) {
    e.target.value = 'LF-' + e.target.value.replace(/^LF-?/, '');
  }
});
...
async function openDetail(id) {
  ...
  $('#detail-content').innerHTML = `
    ...
    <div class="detail-grid">
      ...
      <div><span class="muted">Zuständig</span><br>${escapeHtml(r.assigned_employee_name) || '–'}</div>
    ...
    <form id="compose-form" class="compose-form">
      ...
      <p id="compose-attach-note" class="muted hidden" style="font-size:12px;">📎 Rechnung wird automatisch als PDF angehängt</p>
  ...
  $$('.template-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (!t) return;
      composeAttachInvoice = !!t.attach_invoice;
      $('#compose-attach-note').classList.toggle('hidden', !composeAttachInvoice);
...
async function sendEmail(e, reservation) {
  ...
  btn.disabled = true; btn.textContent = 'Wird gesendet…';
  try {
    await api('send-email', { method: 'POST', body: { to: reservation.customer_email, subject, body, reservationId: reservation.id, attachInvoice: composeAttachInvoice } });
...
async function loadInbox() {
  try {
    ...
    if (unreadCount > lastKnownInboxUnread) notify('Neue E-Mail', 'Ein Kunde hat geantwortet.');
    lastKnownInboxUnread = unreadCount;

    const badge = $('#inbox-badge');
    badge.textContent = unreadCount;
    badge.classList.toggle('hidden', unreadCount === 0);

    $('#inbox-empty').classList.toggle('hidden', channels.length !== 0);
    $('#inbox-list').innerHTML = channels.map((c) => `
      <div class="chat-list-item" data-channel-key="${escapeHtml(c.key)}" data-reservation-id="${c.reservationId || ''}" data-from-address="${escapeHtml(c.fromAddress || '')}">
        <div class="chat-list-info">
          <span class="chat-list-name">${escapeHtml(c.customerName || c.fromAddress || '?')}${c.reservationNumber ? ' · ' + escapeHtml(c.reservationNumber) : ''}</span>
          <span class="chat-list-preview">${escapeHtml(c.lastMessage.body)}</span>
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
          ${c.unreadCount ? `<span class="badge">${c.unreadCount}</span>` : ''}
          <span class="muted" style="font-size:11px;">${fmtDate(c.lastMessage.created_at)}</span>
        </div>
      </div>`).join('');

    $$('#inbox-list .chat-list-item').forEach((el) => {
      el.addEventListener('click', () => openInboxChannel(el.dataset.reservationId || null, el.dataset.fromAddress || null, el.dataset.channelKey));
    });
  } catch (err) {
    console.error(err);
  }
...
let currentInboxChannel = null; // { reservationId, fromAddress }

async function openInboxChannel(reservationId, fromAddress, key) {
  currentInboxChannel = { reservationId: reservationId || null, fromAddress: fromAddress || null };
  $('#inbox-panel-content').innerHTML = `<p class="muted" style="padding:24px 32px;">Lade…</p>`;
  $('#inbox-panel-overlay').classList.remove('hidden');

  try {
    const query = reservationId ? `emails?reservation_id=${reservationId}` : `emails?from_address=${encodeURIComponent(fromAddress)}`;
    const data = await api(query);
    const messages = data.emails || [];
    const unreadIds = messages.filter((m) => m.direction === 'in' && !m.is_read).map((m) => m.id);
    if (unreadIds.length) await api('emails', { method: 'PATCH', body: { ids: unreadIds } });

    const title = messages.find((m) => m.direction === 'in')?.from_address || fromAddress || 'Kunde';
    $('#inbox-panel-content').innerHTML = `
      <div class="staff-chat-header">
        <h2>${escapeHtml(title)}</h2>
      </div>
      <div class="staff-chat-messages" id="inbox-panel-messages">
        ${messages.length ? messages.map((m) => `
          <div class="email-item ${m.direction === 'out' ? 'email-out' : 'email-in'}">
            <div class="email-meta"><strong>${m.direction === 'out' ? 'Gesendet' : 'Empfangen'}</strong><span class="muted">${fmtDate(m.created_at)}</span></div>
            <div class="email-subject">${escapeHtml(m.subject)}</div>
            <div class="email-body">${escapeHtml(m.body)}</div>
          </div>`).join('') : `<p class="muted">Noch keine Nachrichten.</p>`}
      </div>
      <div class="staff-chat-input-row">
        <input type="text" id="inbox-panel-subject" placeholder="Betreff" style="flex:0 0 160px;padding:10px 12px;border-radius:10px;background:var(--surface-2);border:1px solid var(--border);color:var(--text);font-size:13px;" />
        <textarea id="inbox-panel-input" rows="2" placeholder="Antwort schreiben…"></textarea>
        <button id="inbox-panel-send-btn" class="btn-primary" style="width:auto;">Senden</button>
      </div>
    `;
    const msgBox = $('#inbox-panel-messages');
    if (msgBox) msgBox.scrollTop = msgBox.scrollHeight;

    $('#inbox-panel-send-btn').addEventListener('click', async () => {
      const subject = $('#inbox-panel-subject').value.trim() || 'Antwort von LuxeFinds';
      const body = $('#inbox-panel-input').value.trim();
      if (!body) return;
      const to = reservationId ? messages.find((m) => m.direction === 'in')?.from_address : fromAddress;
      try {
        await api('send-email', { method: 'POST', body: { to, subject, body, reservationId } });
        openInboxChannel(reservationId, fromAddress, key);
        loadInbox();
      } catch (err) {
    ...
    $('#inbox-panel-content').innerHTML = `<p class="error-text" style="padding:24px 32px;">Konnte nicht geladen werden.</p>`;
  }

  loadInbox();
}

$('#inbox-panel-close').addEventListener('click', () => {
  $('#inbox-panel-overlay').classList.add('hidden');
  currentInboxChannel = null;
});
...
async function loadLiveChats() {
  try {
    ...
    const totalUnread = chats.reduce((sum, c) => sum + (c.unreadCount || 0) + (c.joinedUnseen ? 1 : 0), 0);

    if (totalUnread > lastKnownChatUnread) notify('Live-Chat', 'Neue Nachricht oder ein Kunde ist beigetreten.');
    lastKnownChatUnread = totalUnread;
    ...
    $('#livechats-list').innerHTML = chats.map((c) => `
      <div class="chat-list-item" data-chat-id="${c.id}">
        <div class="chat-list-info">
          ...
          <span class="chat-list-preview">${c.joinedUnseen ? '🟢 Kunde ist dem Chat beigetreten' : (c.lastMessage ? escapeHtml(c.lastMessage.body) : 'Noch keine Nachricht')}</span>
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
          ${c.unreadCount ? `<span class="badge">${c.unreadCount}</span>` : ''}
          ${c.joinedUnseen ? `<span class="badge" style="background:var(--ok);">neu</span>` : ''}
...
async function refreshStaffChat() {
  ...
  try {
    const data = await api(`chat?chatId=${currentStaffChatId}`);
...
function renderStaffChat(chat, reservation, messages) {
  ...
  if (!staffChatSkeletonBuilt || staffChatSkeletonOpenState !== `${isOpen}-${chat.verified}`) {
    ...
    $('[data-action="send_verification"]')?.addEventListener('click', async (e) => {
      const btn = e.target;
      btn.disabled = true;
      btn.textContent = 'Wird gesendet…';
      try {
        await api('chat', { method: 'POST', body: { chatId: currentStaffChatId, action: 'send_verification' } });
        refreshStaffChat();
      } catch (err) {
        alert(err.message);
        btn.disabled = false;
        btn.textContent = 'Verifizierungscode senden';
      }
    });
...
// ===== Ton & Browser-Benachrichtigungen =====
let lastKnownChatUnread = 0;
let lastKnownInboxUnread = 0;
...
function playNotifySound() {
  ...
  } catch (err) { /* Audio evtl. vom Browser blockiert, ignorieren */ }
}

function notify(title, body) {
  playNotifySound();
  if (window.Notification && Notification.permission === 'granted') {
    try { new Notification(title, { body, icon: 'assets/logo-ring.jpg' }); } catch (err) { /* ignorieren */ }
  }
}
...
$('#enable-notifications-btn')?.addEventListener('click', requestNotificationPermission);
