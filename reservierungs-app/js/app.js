...
function renderStats(list) {
  ...
  const cards = [
    { label: 'Gesamtumsatz', value: euro(total) },
    { label: 'Bereits bezahlt', value: euro(paid) },
    { label: 'Offene Reservierungen', value: open },
  ];
...
function runBootSequence(onDone) {
  ...
  BOOT_STEPS.forEach((step, i) => {
    setTimeout(() => {
      ...
      line.innerHTML = `<span class="running">${escapeHtml(step)}…</span>`;
      linesEl.appendChild(line);
      bar.style.width = `${Math.round(((i + 0.5) / BOOT_STEPS.length) * 100)}%`;

      setTimeout(() => {
        if (finished) return;
        line.innerHTML = `<span>${escapeHtml(step)}</span><span class="ok">OK</span>`;
        bar.style.width = `${Math.round(((i + 1) / BOOT_STEPS.length) * 100)}%`;
        if (i === BOOT_STEPS.length - 1) setTimeout(finish, 350);
      }, 260);
    }, i * 420);
  });
}
